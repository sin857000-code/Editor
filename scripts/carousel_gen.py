...
def slide_cover(data: dict, slide_num: int = 1) -> str:
    ...
    # split headline at accent word for orange highlight
...
def slide_fact(data: dict, slide_num: int = 2) -> str:
    ...
    facts  = data.get("facts", [])  # list of {"key": str, "value": str, "accent": bool}
...
def render_slides(html_list: list[str], output_dir: Path) -> list[Path]:
    """Render list of HTML strings to PNGs using Playwright. Returns list of saved paths."""
...
def build_carousel(slides_data: dict, output_dir: Path) -> list[Path]:
    """
    slides_data keys: cover, fact, player, impact, outro
    Each is a dict consumed by the corresponding slide_* function.
    """
    html_list = [
